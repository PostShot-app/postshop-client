  ...
  if (!stats) {
    return (
      <div className="text-center py-32">
        ...
        <p className="text-white/40">Could not load stats. Check your API connection.</p>
      </div>
    );
  }
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <RevenueCard
          ...
          icon="📈"
          className="bg-ps-dark-card dark:bg-ps-dark-card border border-white/5 dark:border-white/5"
          dark
        />
        <RevenueCard
          title="This Month"
      ...
      <div>
        <div className="flex items-center justify-between mb-5">
          <h2 className="font-heading text-xl font-bold text-white">Recent Sellers</h2>
          <Link href="/admin/sellers" className="text-sm text-ps-orange hover:text-ps-orange-dark font-semibold transition">
            View all →
          </Link>
        </div>

        <div className="bg-ps-dark-card dark:bg-ps-dark-card rounded-2xl border border-white/5 dark:border-white/5 overflow-hidden shadow-warm">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-white/5 dark:border-white/5">
                  <th className="text-left px-5 py-4 font-semibold text-white/40 text-xs uppercase tracking-wider">Shop</th>
                  <th className="text-left px-5 py-4 font-semibold text-white/40 text-xs uppercase tracking-wider hidden sm:table-cell">Phone</th>
                  <th className="text-right px-5 py-4 font-semibold text-white/40 text-xs uppercase tracking-wider">Credits</th>
                  <th className="text-right px-5 py-4 font-semibold text-white/40 text-xs uppercase tracking-wider">Revenue</th>
                  <th className="text-center px-5 py-4 font-semibold text-white/40 text-xs uppercase tracking-wider hidden md:table-cell">Status</th>
                  <th className="text-right px-5 py-4 font-semibold text-white/40 text-xs uppercase tracking-wider hidden lg:table-cell">Joined</th>
                </tr>
              </thead>
              <tbody>
                {sellers.map((s) => (
                  <tr key={s.id} className="border-b border-white/5 hover:bg-white/3 transition">
                    ...
                    <td className="px-5 py-4 text-right text-white font-medium">{s.credits_balance}</td>
                    ...
                    <td className="px-5 py-4 text-right text-white/40 dark:text-white/30 text-xs hidden lg:table-cell">
                      {new Date(s.created_at).toLocaleDateString()}
                    </td>
                  </tr>
                ))}
                {sellers.length === 0 && (
                  <tr><td colSpan={6} className="text-center py-12 text-white/40">No sellers yet</td></tr>
                )}
              </tbody>
            </table>
          </div>
  ...
  return (
    <div className={`rounded-2xl p-6 shadow-warm ${className}`}>
      <div className="flex items-center justify-between mb-3">
        <span className={`text-sm font-medium ${dark ? "text-white/40" : "text-white/70"}`}>{title}</span>
        <span className="text-xl">{icon}</span>
      </div>
      <div className={`font-heading text-2xl sm:text-3xl font-extrabold ${dark ? "text-white" : ""}`}>{value}</div>
      <div className={`text-xs mt-1.5 ${dark ? "text-white/40 dark:text-white/30" : "text-white/50"}`}>{subtitle}</div>
    </div>
  );
}
...
function MetricPill({ label, value, color }: { label: string; value: number; color?: string }) {
  return (
    <div className="bg-ps-dark-card dark:bg-ps-dark-card rounded-2xl border border-white/5 dark:border-white/5 p-5 shadow-warm">
      <div className="text-xs font-medium text-white/40 uppercase tracking-wider">{label}</div>
      <div className={`font-heading text-3xl font-extrabold mt-2 ${color || "text-white"}`}>
